/**
 * Confirmación de Reserva (CR) — HTML → PDF. Portado 1:1 de fincasya-new
 * (pdf.service.buildReservationConfirmationHtml). El PDF se genera con
 * htmlToPdf() (puppeteer). Formato horizontal A4, tabla estilo "letra de cambio".
 */
...
function formatDateDisplay(dateLike: string): string {
...
function formatDateLong(dateLike: string): string {
  const iso = toIsoDate(dateLike);
  if (!iso) return "-";
  const [year, month, day] = iso.split("-");
  const months = [
...
export function buildReservationConfirmationHtml(
  ...
  const logoHtml = opts?.logoDataUrl
    ? `<img src="${opts.logoDataUrl}" alt="FINCASYA LOGO" style="max-width: 250px; max-height: 75px; object-fit: contain;">`
    ...
    <div class="wrapper">
      <div class="header">
        ...
        <div class="header-title-container">
          <div class="header-title">Confirmaci&oacute;n de Reserva N.&ordm; ${escapeHtml(data.contractNumber || "-")}</div>
        </div>
      ...
      <table class="table-1">
        <tr>
          ...
          <td class="value-cell" style="width: 46%;">${escapeHtml(data.clientName || "-")}</td>
          <td class="peach" style="width: 11%;">C. C.</td>
          <td class="value-cell" style="width: 25%;">${escapeHtml(data.clientId || "-")}</td>
        </tr>
        <tr>
          <td class="peach">Correo electr&oacute;nico</td>
          <td class="value-cell">${escapeHtml(data.clientEmail || "-")}</td>
          <td class="peach">Fecha</td>
          <td class="value-cell">${escapeHtml(formatDateDisplay(data.issueDate))}</td>
        </tr>
        <tr>
          <td class="peach">Tel de contacto</td>
          <td class="value-cell">${escapeHtml(data.clientPhone || "-")}</td>
          <td class="peach">DIRECCI&Oacute;N</td>
          <td class="value-cell">${escapeHtml(data.clientAddress || "-")}</td>
        </tr>
      </table>
      ...
      <table class="table-2">
        <tr>
          ...
          <td class="value-cell" style="width: 31%;">${escapeHtml(data.propertyName || "-")}</td>
          <td colspan="2" class="value-cell" style="width: 32%;">Contrato: ${escapeHtml(data.contractNumber || "-")}</td>
          <td rowspan="6" class="empty-box" style="width: 19%;"></td>
        </tr>
        <tr>
          <td class="peach">Ubicaci&oacute;n</td>
          <td colspan="3" class="value-cell">${escapeHtml(data.propertyLocation || "-")}</td>
        </tr>
        <tr>
          <td class="peach">Fecha de Ingreso</td>
          <td class="value-cell">${escapeHtml(formatDateLong(data.checkInDate))}</td>
          <td class="peach" style="width: 16%;">Fecha de Salida</td>
          <td class="value-cell" style="width: 16%;">${escapeHtml(formatDateLong(data.checkOutDate))}</td>
        </tr>
        <tr>
          <td class="peach">Chek In / Chek Out</td>
          <td colspan="3" class="value-cell">${escapeHtml(formatTimeDisplay(data.checkInTime))} / ${escapeHtml(formatTimeDisplay(data.checkOutTime))}</td>
        </tr>
        <tr>
          <td class="peach">Hu&eacute;spedes</td>
          <td class="value-cell">${escapeHtml(String(data.guests || 1))}</td>
          <td class="peach">Noches</td>
          <td class="value-cell">${escapeHtml(String(data.nights || 1).padStart(2, "0"))}</td>
        </tr>
        <tr>
          <td class="peach">Tipo de grupo</td>
          <td class="value-cell">${escapeHtml((data.groupType || "").trim() || "-")}</td>
          <td colspan="2" class="peach">Prop&oacute;sito estancia</td>
          <td class="value-cell">${escapeHtml((data.purpose || "").trim() || "-")}</td>
        </tr>
        <tr>
          <td class="peach">Valor Abono</td>
          <td class="value-cell">${escapeHtml(formatCurrency(data.depositAmount))}</td>
          <td colspan="2" class="peach right-align">Valor Alquiler</td>
          <td class="value-cell">${escapeHtml(formatCurrency(data.rentAmount))}</td>
        </tr>
        <tr>
        ...
        ${
          ...
          <td class="peach"></td>
          <td class="value-cell"></td>
          <td colspan="2" class="peach right-align">${escapeHtml(item.type === "INCREMENT" ? `Ajuste: ${item.description}` : `Descuento: ${item.description}`)}</td>
          <td class="value-cell">${escapeHtml(formatCurrency(item.type === "INCREMENT" ? item.amount : -item.amount))}</td>
        </tr>`,
            )
            .join("") || ""
        }
        <tr>
          ...
          <td class="value-cell">${escapeHtml(formatCurrency(data.balanceAmount))}</td>
          <td colspan="2" class="peach right-align">*Valor Dep&oacute;sito Reembolsable</td>
          <td class="value-cell">${escapeHtml(formatCurrency(data.refundableDeposit))}</td>
        </tr>
        <tr>
          <td class="peach">Fecha Saldo</td>
          <td class="value-cell">${escapeHtml(formatDateLong(data.balanceDate))}</td>
          <td colspan="2" class="peach right-align">Valor TOTAL</td>
          <td class="value-cell">${escapeHtml(formatCurrency(data.totalAmount))}</td>
        </tr>
      </table>
      ...
      <table class="table-banks">
        ...
        <tr class="status-row">
          <td colspan="12">ESTADO DE PAGO: ${escapeHtml(paymentStatusText)}</td>
        </tr>
      </table>
