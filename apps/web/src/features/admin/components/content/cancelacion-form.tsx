...
export function CancelacionManagement() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: CANCELACION_DEFAULT,
  });
  ...
  useEffect(() => {
    async function load() {
      setIsLoading(true);
      try {
        const data = await fetchLegalPage<LegalPageContent>("cancelacion");
        if (data) {
  ...
  async function createFromDefault() {
    setIsSaving(true);
    try {
      await updateLegalPage<FormValues>("cancelacion", CANCELACION_DEFAULT);
      form.reset(CANCELACION_DEFAULT);
      setHasBackendData(true);
  ...
  async function onSubmit(values: FormValues) {
    setIsSaving(true);
    try {
      await updateLegalPage<FormValues>("cancelacion", values);
      setHasBackendData(true);
      sileo.success({
        title: "Política de cancelación actualizada",
        description: "La política de cancelación se guardó correctamente.",
        fill: "#f0fdf4",
      });
    } catch (error) {
      console.error("Error updating cancelación", error);
      sileo.error({
        title: "Error",
        description:
      ...
      <div className="mx-auto max-w-5xl space-y-6 md:space-y-8">
          ...
          <div className="flex items-center gap-3 md:gap-6 w-full md:w-auto">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] md:text-[11px] font-semibold text-muted-foreground uppercase tracking-widest mt-1 md:mt-2 truncate">
                Administrá condiciones de reprogramación y reembolso
              </p>
            </div>
          </div>
          {!hasBackendData ? (
        ...
        <Form {...form}>
          ...
          >
            <FormSection
              title="Hero Legal"
              description="Título y presentación de cancelación"
              icon={FileText}
