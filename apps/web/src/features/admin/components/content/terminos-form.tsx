...
export function TerminosManagement() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: TERMINOS_DEFAULT,
  });
  ...
  useEffect(() => {
    async function load() {
      setIsLoading(true);
      try {
        const data = await fetchLegalPage<LegalPageContent>("terminos");
        if (data) {
  ...
  async function createFromDefault() {
    setIsSaving(true);
    try {
      await updateLegalPage<FormValues>("terminos", TERMINOS_DEFAULT);
      form.reset(TERMINOS_DEFAULT);
      setHasBackendData(true);
  ...
  async function onSubmit(values: FormValues) {
    setIsSaving(true);
    try {
      await updateLegalPage<FormValues>("terminos", values);
      setHasBackendData(true);
      sileo.success({
        title: "Términos actualizados",
        description: "Los Términos y Condiciones se guardaron correctamente.",
        fill: "#f0fdf4",
      });
    } catch (error) {
      console.error("Error updating términos", error);
      sileo.error({
        title: "Error",
        description:
      ...
      <div className="mx-auto max-w-5xl space-y-6 md:space-y-8">
          ...
          <div className="flex items-center gap-3 md:gap-6 w-full md:w-auto">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] md:text-[11px] font-semibold text-muted-foreground uppercase tracking-widest mt-1 md:mt-2 truncate">
                Administrá el contenido legal de la plataforma
              </p>
            </div>
          </div>
          {!hasBackendData ? (
        ...
        <Form {...form}>
          ...
          >
            <FormSection
              title="Hero Legal"
              description="Título y presentación de los términos"
              icon={FileText}
